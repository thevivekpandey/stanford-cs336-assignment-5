"""
GRPO training script for OLMo-2-0425-1B on GSM8K, using the question_only
prompt instead of r1_zero.

Same two-GPU setup as train_grpo.py (one GPU for vLLM rollouts, one for
training). The learning rate is fixed here; the sweep axis is the random seed
(see sweep_seeds.sh).
"""
...
RANDOM_SEED = 42  # default; override with --seed (see sweep_seeds.sh)
MODEL_NAME = "allenai/OLMo-2-0425-1B"
DATASET_PATH = "../data/gsm8k"
PROMPT_PATH = "prompts/question_only.prompt"
REWARD_FN = question_only_reward_fn  # must match PROMPT_PATH

# Training hyperparameters
N_TRAIN_EXAMPLES = 6400
N_VAL_EXAMPLES = 1024
NUM_ROLLOUT_STEPS = 250  # override with --num-rollout-steps
LEARNING_RATE = 1e-5  # fixed for this experiment; the sweep axis is the seed
...
SAVE_EVERY = 100
CHECKPOINT_DIR = "checkpoints/grpo_question_only"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="GRPO training on GSM8K with the question_only prompt."
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=RANDOM_SEED,
        help="Random seed. Seeds torch (batch sampling) and the vLLM engine "
             "(rollout sampling).",
    )
    parser.add_argument(
        "--num-rollout-steps",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="WandB run name. Defaults to grpo_question_only_seed=<seed>.",
    )
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
        default=None,
        help="Checkpoint directory. Defaults to <CHECKPOINT_DIR>/seed_<seed>.",
    )
    args = parser.parse_args()
    if args.run_name is None:
        args.run_name = f"grpo_question_only_seed={args.seed}"
    if args.checkpoint_dir is None:
        args.checkpoint_dir = str(Path(CHECKPOINT_DIR) / f"seed_{args.seed}")
    return args
...
def main():
    ...
    # Seed torch: controls which training examples each step samples.
    ...
    # Plot everything against the rollout step, not wandb's internal counter.
    # That counter increments once per wandb.log() call, and a single rollout
    # step can emit up to three (train metrics, eval, rollout table), so the
    # default x-axis overcounts and misaligns the train and eval curves.
    ...
    # Seed at the engine level, not per-request: a per-request seed in
    # SamplingParams would make all group_size rollouts of the same prompt
    # identical, collapsing the GRPO advantages to zero.
    vllm_engine = LLM(
        model=MODEL_NAME,
    ...
    for step in range(num_rollout_steps):
        ...
        if step % LOG_ROLLOUTS_EVERY == 0:
            ...
            wandb.log({"step": step, "rollouts": rollout_table})
